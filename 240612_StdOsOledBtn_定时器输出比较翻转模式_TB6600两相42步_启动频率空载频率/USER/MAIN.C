...
{		
	...
	ASSERT(pdPASS == xReturn);
	xReturn = xTaskCreate(StepMotor_task,"StepMotor_task",512,NULL,10,&g_StepMotorTask_Handler);	
	ASSERT(pdPASS == xReturn);
	taskEXIT_CRITICAL();
	vTaskDelete(NULL);
}
...
void oled_task(void *argv)
{
	...
	while (1)
	{
		...
		//len = OLED_ShowString(0, 6,"ST_CTRL:");
		//OLED_PrintSignedVal(len, 6, SysTick->CTRL );

		snprintf (str_temp,sizeof(str_temp),"ST_L:0X%x",SysTick->LOAD);
		OLED_ShowString(0, 4, str_temp);

		snprintf (str_temp,sizeof(str_temp),"ST_V:0X%x",SysTick->VAL);
