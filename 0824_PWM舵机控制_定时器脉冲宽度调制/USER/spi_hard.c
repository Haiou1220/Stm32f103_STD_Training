...
void spi1_hard_init(u8 mode) // spi1硬件初始化,模式0  NSS 软件模式 ,//PA4 -NSS //PA5-SCK  //PA6-MI  //PA7-MO
{
	uint16_t SPI_CPHA_XEdge,SPI_CPOL_LowHigh;

	if(mode == 0)
	{
		   SPI_CPOL_LowHigh = SPI_CPOL_Low  ;
			SPI_CPHA_XEdge = SPI_CPHA_1Edge ;
	}else if( mode == 1)
	{
		 SPI_CPOL_LowHigh = SPI_CPOL_Low ;
		 SPI_CPHA_XEdge   =  SPI_CPHA_2Edge;
	}else if( mode == 2)
	{
	    SPI_CPOL_LowHigh = SPI_CPOL_High ;
		SPI_CPHA_XEdge   = SPI_CPHA_1Edge ;
	}else if( mode == 3)
	{
		   SPI_CPOL_LowHigh = SPI_CPOL_High ;
	...
	SPI_InitStruct.SPI_CPHA = SPI_CPHA_XEdge; //模式X
	SPI_InitStruct.SPI_CPOL = SPI_CPOL_LowHigh; //模式X
