...
int SSD1306_WriteCmd(uint8_t cmd)
{
    ...
    byte_buf[0] = CTRL_BYTE_CMD; // Co=0:仅发送数据字节,控制字节不发送       D/C#=0;发命令
    byte_buf[1] = cmd;
   
	return I2C_Master_Transmit(I2C1,SSD1306_I2C_ADDR,byte_buf,sizeof(byte_buf),OLED_TIMEOUT);    
}
...
int SSD1306_WriteData(uint8_t data)
{
    ...
    *p_ctrl_byte = CTRL_BYTE_DATA; // D/C#=1;发数据
    *p_data_byte = data;    
...
int SSD1306_WriteNBytes(uint8_t *buf, uint16_t length)
{
    ...
    return I2C_Mem_Write(I2C1, SSD1306_I2C_ADDR, CTRL_BYTE_DATA, OLED_MEM_ADD_SIZE, buf, length, OLED_TIMEOUT);
}
