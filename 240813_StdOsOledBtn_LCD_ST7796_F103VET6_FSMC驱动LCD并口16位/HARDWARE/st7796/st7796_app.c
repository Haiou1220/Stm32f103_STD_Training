...
SemaphoreHandle_t g_st7796_mutex_handle = NULL; //export
...
void st7796_task(void *pvarg)//修改驱动为并口 fsmc,todo,st7796_spi_lcd模块的spi 移过来测试 
{
    uint32_t count = 0;
    char str[20];
    g_st7796_mutex_handle = xSemaphoreCreateMutex();//创建互斥信号量
    ASSERT(NULL != g_st7796_mutex_handle);

    printf(" in st7796_task LCD_BASE :%x\r\n",LCD_BASE);   

    st7796_bsp_init();
    st7796_init(); //写入出厂配置,设置屏幕亮度,ok

    st7796_Display_device_init(0); //配置屏幕方向与尺寸,ok
 ...
 // st7796_ShowChar(0,0,*p,12,0);
//st7796_ShowString(10,50,200,12,12,"6X12:abcdefghijklmnopqrstuvwxyz");
 
   st7796_backlight_duty(50);
    ...
    while (1)
    {       
//st7796_ShowString(10,50,200,12,12,"6X12:1234567890987654321123456");        
//st7796_ScreenClear(RGB_BLUE);
// vTaskDelay(pdMS_TO_TICKS(500));  
//  //st7796_ScreenFill(0,0, st7796_get_lcddev_width(), st7796_get_lcddev_height(), RGB_BLUE);
// st7796_ScreenClear(RGB_BLUE);

//     // vTaskDelay(pdMS_TO_TICKS(1000));     
//      //printf(" in st7796_task:%d\r\n",count++);   
// st7796_ShowString(10,50,200,24*2,24,"6X12:abcdefghijklmnopqrstuvwxyz");

// //st7796_ScreenClear(RGB_YELLOW);
// vTaskDelay(pdMS_TO_TICKS(500));  
// //st7796_ScreenFill(0,0, st7796_get_lcddev_width(),st7796_get_lcddev_height(), RGB_YELLOW);
//  st7796_ScreenClear(RGB_YELLOW);

        for(y=0;(y<st7796_get_lcddev_height())&&((y+size)<st7796_get_lcddev_height());y+=size)
        {
            ...
            snprintf(str,sizeof(str),"g_oled_count:%d\r\n",g_oled_count);       
            st7796_ShowString(x,y,x_leng,size,size,str);
            vTaskDelay(pdMS_TO_TICKS(500));
            st7796_ScreenFill(x,y,x+x_leng,y+size,RGB_BLACK);

        }
        st7796_ShowNum(x_leng+5,0,for_count++,5,size);//?????????????????????
    }
