...
void key0_interrupt_init()//key0 pe4 配置外部中断触发
{	
	...
	GPIO_InitStruct.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(  GPIOE, &GPIO_InitStruct);//配置gpio
	GPIO_EXTILineConfig(GPIO_PortSourceGPIOE, GPIO_PinSource4);//配置中断线映射afio
	EXTI_InitTypeDef   EXTI_InitStruct;
	EXTI_InitStruct.EXTI_Line = EXTI_Line4;
	EXTI_InitStruct.EXTI_LineCmd = ENABLE;
	EXTI_InitStruct.EXTI_Mode = EXTI_Mode_Interrupt;
	EXTI_InitStruct.EXTI_Trigger = EXTI_Trigger_Falling;
	EXTI_Init(&EXTI_InitStruct);//配置中断事件,下降沿触发
	NVIC_InitTypeDef   NVIC_InitStruct;
...
int main(void)
{
 ...
 NVIC_PriorityGroupConfig(  NVIC_PriorityGroup_2);//配置中断分组
  usart1_init(115200 ) ;//pa9 tx,  pa10 rx
   key0_interrupt_init();//key0 pe4 配置外部中断触发
